...
export function FlipCard({
  ...
  return (
    <div
      className={`relative grid h-[100px] w-[78px] grid-rows-[1fr_auto_1fr] overflow-hidden rounded-[6px_6px_9px_9px] sm:h-[104px] sm:w-[86px] ${bg} ${shadow} ${gamePoint ? 'ring-2 ring-[#e0a92a]' : ''}`}
    >
      ...
      <button
        ...
        onClick={() => step(1)}
        className={`flex items-start justify-center pt-1 text-sm font-bold text-[#e0a92a]/70 transition-opacity ${cuesHidden ? 'opacity-0' : 'opacity-100'}`}
      >
        +
      </button>
      ...
      <div className="group relative grid place-items-center">
        ...
        <span
          key={flip}
          aria-hidden
          className={`${flip > 0 ? 'flip-card-anim ' : ''}pointer-events-none border-b-2 border-transparent pb-0.5 font-mono nums text-[40px] font-bold leading-none group-focus-within:border-[#e0a92a] sm:text-[44px] ${
        ...
        <input
          ...
          className="absolute inset-0 h-full w-full bg-transparent text-center font-mono text-[40px] font-bold leading-none text-transparent caret-[#1b1d22] focus:outline-none sm:text-[44px] [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
        />
      </div>
      ...
      <button
        ...
        onClick={() => step(-1)}
        className={`flex items-end justify-center pb-1 text-sm font-bold text-[#e0a92a]/55 transition-opacity ${cuesHidden ? 'opacity-0' : 'opacity-100'}`}
      >
        −
      </button>
